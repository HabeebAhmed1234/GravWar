...
public class LevelGenerator {
	...
	private void getEnemyPlanets()
	{
		...
		while(totalMissiles>0)
		{
			...
			Planet newEnemyPlanet = new Planet(getUniquePlanetId(), 0, 0, missilesPerPlanet * Constants.PLANET_HEALTH_IN_MISSILES_TO_DIAMETER_RATIO, new PlanetType(PlanetType.PLANET_TYPE_ENEMY), this.vertexBufferObjectManager, gameManager, gameScene, gametextureManager);
	...
	private void getPlayerPlanets()
	{
		...
		{
			totalMissiles-=missilesPerPlanet;
			Planet newEnemyPlanet = new Planet(getUniquePlanetId(), 0, 0, missilesPerPlanet * Constants.PLANET_HEALTH_IN_MISSILES_TO_DIAMETER_RATIO, new PlanetType(PlanetType.PLANET_TYPE_PLAYER), vertexBufferObjectManager, gameManager,gameScene, gametextureManager);
	...
	private Position getLegalPosition(boolean isEnemy, float diameter)
	{
		...
		if(!isEnemy)
		{
			while(y-radius<0 || y+radius>m_midwayLine || x-radius<0 || x+radius>m_screenWidth || isCollidingWithAnyPlanet(x,y,radius))
			{
		...
		{
			while(y-radius<m_midwayLine || y+radius>m_screenHeight || x-radius<0 || x+radius>m_screenWidth || isCollidingWithAnyPlanet(x,y,radius))
			{
				x = (float)(Math.random()*m_screenWidth);
				y = (float)(Math.random()*m_screenHeight);
			}
	...
	private boolean isCollidingWithAnyPlanet(float x, float y, float r)
	{
		r = (Constants.PLANET_MAX_HEALTH*Constants.PLANET_HEALTH_IN_MISSILES_TO_DIAMETER_RATIO)/2+10;
		for(int i = 0 ; i < m_levelPlanets.size(); i++ )
		{
			Planet tempPlanet = m_levelPlanets.get(i);
			if(Utilities.circlesColliding(x,y,r,tempPlanet.getPosition().getX(),tempPlanet.getPosition().getY(),(Constants.PLANET_MAX_HEALTH*Constants.PLANET_HEALTH_IN_MISSILES_TO_DIAMETER_RATIO)/2+10)) return true;
		}
		return false;
	}
}
